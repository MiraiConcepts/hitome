...
| 14 | Midnight-spanning timed events · 2026-08-02 | **The day begins at 06:00**: a timed event ending in the small hours (00:00 < end ≤ 06:00) claims only its start day — no month-grid banner, no widget continuation row (a late night out is one evening, not two days). Ends past 06:00 keep the multi-day span; the widget's final continuation day then leads with `→ HH:MM` (the end time) instead of the all-day sun, so the sun always means a genuinely covered day. Shared rule: `eventLastMs` (utils/date) feeds both `eventDays` and the month-grid layout. |
